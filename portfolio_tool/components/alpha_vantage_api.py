...
class AlphaVantageAPI:
    ...
    def security_date_price(self, ticker, dates):
        data = self.security_price(ticker, endpoint='weekly_time_series')
    ...
    def fx_rates(self, curr_from, curr_to, dates=[]):
        ...
        curr_to -- The currency to exchange to
        '''
    ...
    # --- Pandas Wrappers --- #
    def get_currency_df(self, input_df):
        ...
        for currency in currencies:
            ...
            fx = self.fx_rates(currency, 'USD', dates=fx_dates)
            fx = pd.DataFrame({'as_of_date':fx.keys(), 'fx_rate':fx.values()})
            fx['currency'] = currency
            fx_list.append(fx)

            self.sleep_tracker += 1
            if self.sleep_tracker == 5:
                print("waiting 1 min, API limit")
                time.sleep(60)
                self.sleep_tracker = 0
    ...
    def get_ticker_price_df(self, input_df):
        df = input_df.copy()
        # Hit API with ticker to autopopulate closing price
        tickers = df['ticker'].unique().tolist()
        tickers = [i for i in tickers if i != 'none']
        tickers = [i for i in tickers if i != 'cash']
        tickers = [i for i in tickers if i != 'cash_untapped']

        price_list = []
        
        for ticker in tickers:
            ticker_dates = df[df['ticker']==ticker]['as_of_date'].unique().tolist()
            price = self.security_date_price(ticker, dates=ticker_dates)
            ...
            print(f"Got ticker price: {ticker.upper()}")
...
if __name__ == '__main__':
    ...
    AlphaVantageAPI.security_date_price('BRK-B', dates=['2023-01-15', '2022-01-15'])
